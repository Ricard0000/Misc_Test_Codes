# -*- coding: utf-8 -*-
"""
Physics informed NN reduced dimensions.
"""
...
class NeuralNetworkCode:
    def __init__(self,v_flat,x_flat,t_flat,g_flat,rho_flat,layers,Losss,wv_flat,vrhox_flat,gt_flat,mask_flat):
        ...
        self.g_tf = tf.placeholder(tf.float32, shape=[Nv*Nx,(n-2*pad)])
        self.rho_tf = tf.placeholder(tf.float32, shape=[Nv*Nx,(n-2*pad)])
...
#        self.loss =tf.reduce_mean(tf.abs(self.g_pred-self.g_tf))+tf.reduce_mean(tf.abs(0.0 - self.D1))+tf.reduce_mean(tf.abs(0.0 - self.D2))
    ...
    def neural_net(self, weights, biases, D1, D2):
        ...
        print(weights[num_layers-1].shape)
        print(weights[num_layers-1].shape)
        print(weights[num_layers-1].shape)        
        ...
        b = biases[l]
        l=l+1
        Hout2 = tf.add(tf.matmul(H, W), b)
        print('2')
        print('2')                
        H3=tf.concat([Hout1,Hout2], 1)
        for I in range(2,n-2*pad):
            V=D1+self.v_flat
            X=D2+self.x_flat
            H=tf.concat([V,X], 1)
            for J in range(0,num_layers-2):
            ...
            b = biases[l]
            l=l+1
            Hout = tf.add(tf.matmul(H, W), b)

            H3=tf.concat([H3,Hout], 1)
        print('3')
        print('3')            
        print(H3.shape)
        print(H3.shape)
    ...
    def net_uv(self, x_flat ):
        g = self.neural_net(self.weights, self.biases, self.D1, self.D2)


        g_v1=tf.gradients(g[:,0],self.D1)[0]
        g_v2=tf.gradients(g[:,1],self.D1)[0]
        conc1=tf.concat([g_v1,g_v2], 1)
        for I in range(2,n-2*pad):
            g_v3=tf.gradients(g[:,I],self.D1)[0]
            conc1=tf.concat([conc1,g_v3], 1)


        g_x1=tf.gradients(g[:,0],self.D2)[0]
        g_x2=tf.gradients(g[:,1],self.D2)[0]
        conc2=tf.concat([g_x1,g_x2], 1)
        for I in range(2,n-2*pad):
            g_x3=tf.gradients(g[:,I],self.D2)[0]
            conc2=tf.concat([conc2,g_x3], 1)
...
#        bc=g*self.mask_flat
        return g,conc1,conc2,conc2,conc1
...
if __name__ == "__main__": 
    ...
    L=-1
    gt_flat=np.zeros([Nv*Nx,(n-2*pad)],dtype=float)
    for I in range(0,Nv):
        for J in range(0,Nx):
            L=L+1
            for K in range(pad,n-pad):
                gt_flat[L,K-pad]=partial_tg[I,J,K]


    #Setting boundary condition to zero
    
    mask=np.zeros([Nv,Nx,n-2*pad],dtype=float)
    mask_flat=np.zeros([Nv*Nx,(n-2*pad)],dtype=float)
    L=-1
#    for I in range(0,Nv):
...
#                L=L+1
    ...
    t_flat=np.zeros([Nv*Nx,(n-2*pad)],dtype=float)
    g_flat=np.zeros([Nv*Nx,(n-2*pad)],dtype=float)
    rho_flat=np.zeros([Nv*Nx,(n-2*pad)],dtype=float)
    wv_flat=np.zeros([Nv*Nx,(n-2*pad)],dtype=float)
    ...
    for I in range(0,Nv):
        for J in range(0,Nx):
            L=L+1
            for K in range(pad,n-pad):
                t_flat[L,K-pad]=t[K,0]
                g_flat[L,K-pad]=g[I,J,K]
                rho_flat[L,K-pad]=rho[I,J,K]
                wv_flat[L,K-pad]=wv[I,0]
...
##Rho does not get NN fitting for fairness:
    vrhox_flat=np.zeros([Nv*Nx,(n-2*pad)],dtype=float)
    ...
    for I in range(0,Nv):
        for J in range(0,Nx):
            L=L+1
            for K in range(pad,n-pad):
                vrhox_flat[L,K-pad]=v[I]*rhox[I,J,K]
    ...
    for I in range(0,Nv):
        for J in range(0,Nx):
            L=L+1
            for K in range(0,(n-2*pad)):
                g_pred_3d[I,J,K]=g_pred[L,K]
                gv_pred_3d[I,J,K]=gv_pred[L,K]
                gx_pred_3d[I,J,K]=gx_pred[L,K]
                gt_pred_3d[I,J,K]=gt_pred[L,K]
    ...
    ax2.plot_wireframe(v_mesh,x_mesh, g_pred_3d[:,:,0], color='r')
    ...
    fig = plt.figure()
    ax2 = plt.axes(projection='3d')
    ax2.plot_wireframe(v_mesh,x_mesh, gv_pred_3d[:,:,2], color='r')
    plt.title('Neural Network Solution',fontsize=14,fontweight='bold')
    
    fig = plt.figure()
    ax2 = plt.axes(projection='3d')
    ax2.plot_wireframe(v_mesh,x_mesh, gv[:,:,2], color='r')
    plt.title('Exact Solution',fontsize=14,fontweight='bold')
    ...
    fig = plt.figure()
    ax2 = plt.axes(projection='3d')
    ax2.plot_wireframe(v_mesh,x_mesh, gx_pred_3d[:,:,2], color='r')
    plt.title('Neural Network Solution',fontsize=14,fontweight='bold')
    
    fig = plt.figure()
    ax2 = plt.axes(projection='3d')
    ax2.plot_wireframe(v_mesh,x_mesh, gx[:,:,2], color='r')
    plt.title('Exact Solution',fontsize=14,fontweight='bold')


    fig = plt.figure()
    ax2 = plt.axes(projection='3d')
    ax2.plot_wireframe(v_mesh,x_mesh, gt_pred_3d[:,:,10], color='r')
    plt.title('Neural Network Solution',fontsize=14,fontweight='bold')
    
    fig = plt.figure()
    ax2 = plt.axes(projection='3d')
    ax2.plot_wireframe(v_mesh,x_mesh, gt[:,:,10], color='r')
    plt.title('Exact Solution',fontsize=14,fontweight='bold')
