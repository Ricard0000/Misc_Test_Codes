...
class PDE_CLASSIFY:
    def __init__(self,g,rho_extend,n_dat,N_layers,Nx,dt,hx,max_epsilon,min_epsilon,Max_orders,Losss,x1,x2,x3):
        self.g=g
...
#        self.loss = tf.reduce_mean(tf.abs(self.g_pred))
        ...
        self.loss = tf.reduce_mean(tf.abs(self.g_pred))+tf.reduce_mean(tf.abs(self.sigmaS[0]-0))#+gamma*(tf.reduce_mean(tf.abs(self.cont0))+tf.reduce_mean(tf.abs(self.cont1))+tf.reduce_mean(tf.abs(self.cont2))+tf.reduce_mean(tf.abs(self.diff1))+tf.reduce_mean(tf.abs(self.diff2)))#+tf.reduce_mean(tf.abs(self.sigmaS[1]-(1+100*x[1]**2)))+tf.reduce_mean(tf.abs(self.sigmaS[Nx-2]-(1+100*x[Nx-2]**2)))+tf.reduce_mean(tf.abs(self.sigmaS[Nx-1]-(1+100*x[Nx-1]**2)))#+0.000001*tf.reduce_mean(tf.abs(self.diff[1:Nx-1]))#+tf.reduce_mean(tf.abs(self.sigmaS[Nx-1]-100))#+gamma*tf.reduce_mean(tf.abs(self.g_innerproduct))# +gamma*(tf.reduce_mean(tf.abs(self.w_110_tf[0]))+tf.reduce_mean(tf.abs(self.w_120_tf[0]))+tf.reduce_mean(tf.abs(self.w_111_tf[0]))+tf.reduce_mean(tf.abs(self.w_121_tf[0]))+tf.reduce_mean(tf.abs(self.w_112_tf[0]))+tf.reduce_mean(tf.abs(self.w_122_tf[0])))+gamma*(tf.reduce_mean(tf.abs(self.w_110_tf[1]))+tf.reduce_mean(tf.abs(self.w_120_tf[1]))+tf.reduce_mean(tf.abs(self.w_111_tf[1]))+tf.reduce_mean(tf.abs(self.w_121_tf[1]))+tf.reduce_mean(tf.abs(self.w_112_tf[1]))+tf.reduce_mean(tf.abs(self.w_122_tf[1])))
...
#        sigmaS5=self.a4*x5**2+self.b4*x5+self.c4
        ...
        #Set up continutity
        cont0=self.a0*x1[0]**2+self.b0*x1[0]+self.c0
...
#    x1=np.linspace(0,1.0-hx,Nx)
    x1=np.linspace(0,1.0/3.0-hx,int(Nx/3))
    x2=np.linspace(1.0/3.0,2.0/3.0-hx,int(Nx/3))
    x3=np.linspace(2.0/3.0,1.0-hx,int(Nx/3))
    x1=np.float32(x1)
    x2=np.float32(x2)
    x3=np.float32(x3)
    
    modelrho = PDE_CLASSIFY(g,rho_extend,n_dat,N_layers,Nx,dt,hx,max_epsilon,min_epsilon,Max_orders,Losss,x1,x2,x3)
