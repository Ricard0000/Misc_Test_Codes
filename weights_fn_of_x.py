...
class PDE_CLASSIFY:
    def __init__(self,g,rho_extend,n_dat,N_layers,Nx,dt,hx,max_epsilon,min_epsilon,Max_orders,Losss):
        ...
        self.w_110,self.w_120,self.w_111,self.w_121,self.w_112,self.w_122,self.w_eps, self.wstiffg2, self.sigmaS, self.sigmaA, self.cont_const= self.initialize_NNg(N_layers)
        
        # tf Graphs
        self.g_pred,self.w_110_tf, self.w_120_tf,self.w_111_tf, self.w_121_tf,self.w_112_tf, self.w_122_tf,self.w_eps_tf,self.wstiff_g2_tf,self.g_innerproduct,self.rho_pred,self.eps_pred,self.sigmaS_tf,self.sigmaA_tf, self.diff = self.net_uv(self.g_tf,self.rho_extend_tf)
        # Loss
...
#        self.loss = tf.reduce_mean(tf.abs(self.g_pred))
        ...
        self.loss = tf.reduce_mean(tf.abs(self.g_pred))+tf.reduce_mean(tf.abs(self.sigmaS[0]-1))+1*tf.reduce_mean(tf.abs(self.diff[2:Nx-2]-self.cont_const))#+tf.reduce_mean(tf.abs(self.sigmaS[1]-(1+100*x[1]**2)))+tf.reduce_mean(tf.abs(self.sigmaS[Nx-2]-(1+100*x[Nx-2]**2)))+tf.reduce_mean(tf.abs(self.sigmaS[Nx-1]-(1+100*x[Nx-1]**2)))#+0.000001*tf.reduce_mean(tf.abs(self.diff[1:Nx-1]))#+tf.reduce_mean(tf.abs(self.sigmaS[Nx-1]-100))#+gamma*tf.reduce_mean(tf.abs(self.g_innerproduct))# +gamma*(tf.reduce_mean(tf.abs(self.w_110_tf[0]))+tf.reduce_mean(tf.abs(self.w_120_tf[0]))+tf.reduce_mean(tf.abs(self.w_111_tf[0]))+tf.reduce_mean(tf.abs(self.w_121_tf[0]))+tf.reduce_mean(tf.abs(self.w_112_tf[0]))+tf.reduce_mean(tf.abs(self.w_122_tf[0])))+gamma*(tf.reduce_mean(tf.abs(self.w_110_tf[1]))+tf.reduce_mean(tf.abs(self.w_120_tf[1]))+tf.reduce_mean(tf.abs(self.w_111_tf[1]))+tf.reduce_mean(tf.abs(self.w_121_tf[1]))+tf.reduce_mean(tf.abs(self.w_112_tf[1]))+tf.reduce_mean(tf.abs(self.w_122_tf[1])))
#        self.loss = tf.reduce_mean(tf.abs(self.g_pred))+gamma*tf.reduce_mean(tf.abs(self.g_innerproduct-projection_term[:,1:n_dat-1]))# +gamma*(tf.reduce_mean(tf.abs(self.w_110_tf[0]))+tf.reduce_mean(tf.abs(self.w_120_tf[0]))+tf.reduce_mean(tf.abs(self.w_111_tf[0]))+tf.reduce_mean(tf.abs(self.w_121_tf[0]))+tf.reduce_mean(tf.abs(self.w_112_tf[0]))+tf.reduce_mean(tf.abs(self.w_122_tf[0])))+gamma*(tf.reduce_mean(tf.abs(self.w_110_tf[1]))+tf.reduce_mean(tf.abs(self.w_120_tf[1]))+tf.reduce_mean(tf.abs(self.w_111_tf[1]))+tf.reduce_mean(tf.abs(self.w_121_tf[1]))+tf.reduce_mean(tf.abs(self.w_112_tf[1]))+tf.reduce_mean(tf.abs(self.w_122_tf[1])))
 ...
 #       self.loss =tf.reduce_mean(tf.abs(self.g_innerproduct))#+1.0/(gamma*(tf.reduce_mean(tf.abs(self.w_110_tf[0]))+tf.reduce_mean(tf.abs(self.w_120_tf[0]))+tf.reduce_mean(tf.abs(self.w_111_tf[0]))+tf.reduce_mean(tf.abs(self.w_121_tf[0]))+tf.reduce_mean(tf.abs(self.w_112_tf[0]))+tf.reduce_mean(tf.abs(self.w_122_tf[0])))+gamma*(tf.reduce_mean(tf.abs(self.w_110_tf[1]))+tf.reduce_mean(tf.abs(self.w_120_tf[1]))+tf.reduce_mean(tf.abs(self.w_111_tf[1]))+tf.reduce_mean(tf.abs(self.w_121_tf[1]))+tf.reduce_mean(tf.abs(self.w_112_tf[1]))+tf.reduce_mean(tf.abs(self.w_122_tf[1]))))
    ...
    def initialize_NNg(self, N_layers):
        ...
        return w_110,w_120,w_111,w_121,w_112,w_122,w_eps,wstiffg2,sigmaS,sigmaA,cont_const
    ...
    def net_uv(self,g,rho_extend):
        ...
        print(g.shape)
        """
        sigmaS=unknown(usually sparse less thatn Nx points, say Nx/5)
        s1=D and C depend on unknown function *x
        s2=
        s3=
        s4=
        sigmaS_spline=concate(s1 through s4)(now this is a function of x with Nx points)
        """
...
#        diff_sigmaS=(-0.5*tf.roll(sigmaS,2,0)+tf.roll(sigmaS,1,0)-tf.roll(sigmaS,-1,0)+0.5*tf.roll(sigmaS,-2,0))#/(hx**3)
        ...
        return K_g,w_110,w_120,w_111,w_121,w_112,w_122,w_eps,wstiffg2,Frhs_innerproduct,K_rh,eps,sigmaS,sigmaA,diff_sigmaS
...
#    TTdata = scipy.io.loadmat('data_mm_periodic.mat')
    ...
    x1=np.linspace(0,0.2,int(Nx/5))
    x2=np.linspace(0.2,0.4,int(Nx/5))
    x3=np.linspace(0.4,0.6,int(Nx/5))
    x4=np.linspace(0.6,0.8,int(Nx/5))
    x5=np.linspace(0.8,1,int(Nx/5))
...
#    savemat('W_stiff_bdf2',{'wstiffg2':wstiffg2})
    ...
    fig = plt.figure()
    plt.scatter(axiss,sigmaS, color='black', linewidth=2, label='sigma S')
    plt.show()
